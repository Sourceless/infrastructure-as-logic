...
?- resource(NAME, TYPE, _PARAMS), get_dict(bucket, _PARAMS, "my-tf-test-bucket").
NAME=example
TYPE=aws_s3_bucket
```
...
In a real system where multiple parties want to modify the same resources, we will need a more composable update model.

To do this, we'll tear several pages out of Datomic's book, and instead of modeling facts directly, we'll model them as *datoms*. Datoms are an `assert` (create or update) or `retract` (delete) followed by an identifier, some data, and a time. This gives us several powerful capabilities:
* Resources can be built up incrementally
* Partial updates are explicit and conflict-aware
* Multiple independent rules can safely contribute to the same resource
* Deletions are represented directly, not just by absence

We'll explore this model in more detail in Part II: Infrastructure as Data.
...
* When something existing is updated
* When something existing is deleted

### Finding updated resources
Fortunately, finding a updated resource is not too hard:
```prolog
updated(NAME, TYPE, PARAMS, PARAMS_NEXT) :-
    resource(existing, NAME, TYPE, PARAMS),
    resource(desired, NAME, TYPE, PARAMS_NEXT),
    PARAMS =\= PARAMS_NEXT.
```

When we query `updated`, we are now returned a list of the resources that:
* Already exist
